...
class VideoAnnotator:
    ...
    Raises:
        ValueError: If neither labels_dir nor predictions is provided, or images_dir is empty.
    ...
    def __init__(
        ...
        if labels_dir is None and predictions is None:
            raise ValueError("At least one of labels_dir or predictions must be provided.")
        ...
        # Determine mode
        has_gt = self._labels_dir is not None
        ...
        if frame_map_path is not None:
            ...
            with open(frame_map_path, newline="") as f:
                ...
                for row in reader:
                    frame_stem = Path(row["frame"]).stem          # e.g. "frame_000000"
                    video_stem = Path(row["video"]).stem           # e.g. "IMG_0231"
