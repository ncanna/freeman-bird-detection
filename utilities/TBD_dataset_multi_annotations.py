...
#         But I did not find an example usage so will skip for now
#         '''
        
#         pass


# TODO: There are a few frames with multiple annotations, which will break the defualt DataLoader
#       RuntimeError: each element in list of batch should be of equal size
# def collate_fn(batch):
#     """
#     Custom collate function to handle variable number of annotations per image.
#     Default collate fails because each image can have different numbers of birds.
...
#     """
#     images, annotations = zip(*batch)       # unzip list of (image, anno) tuples
#     images = torch.stack(images, dim=0)     # stack images into [batch_size, 3, H, W]
#     annotations = list(annotations)         # keep annotations as a list (not tensor)
