...
class YOLOAdapter(BaseModelAdapter):
    ...
    def prepare_data(
        ...
        self._data_yaml_path = build_ultralytics_dataset(
            dataset_manager, config, self.work_dir, "yolo.yaml"
        )
    ...
    def _discover_data_yaml(self, config: "ExperimentConfig") -> None:
        ...
        Priority:
          ...
          2. config.resume_experiment_name work dir — existing resume-training flow
        """
        ...
        if config.resume_experiment_name is None:
            raise RuntimeError(
                "yolo.yaml not found in work_dir and resume_experiment_name is not set; "
